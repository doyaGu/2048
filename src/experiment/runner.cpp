...
std::string ResolvedConfigToml(const ExperimentProfile& profile) {
    ...
    out << "[run]\n"
        ...
        << "approximate_chance_nodes = " << (profile.search.approximateChanceNodes ? "true" : "false") << "\n"
        << "max_chance_branches_per_value = " << profile.search.maxChanceBranchesPerValue << "\n"
        << "preserve_chance_probability_mass = "
        << (profile.search.preserveChanceProbabilityMass ? "true" : "false") << "\n"
        << "adaptive_endgame_search = " << (profile.search.adaptiveEndgameSearch ? "true" : "false") << "\n"
        << "endgame_min_rank = " << profile.search.endgameMinRank << "\n"
        << "endgame_depth_bonus = " << profile.search.endgameDepthBonus << "\n"
        << "endgame_max_chance_branches_per_value = " << profile.search.endgameMaxChanceBranchesPerValue << "\n"
        << "endgame_pessimism = " << profile.search.endgamePessimism << "\n"
        << "canonicalize_transposition_keys = "
        << (profile.search.canonicalizeTranspositionKeys ? "true" : "false") << "\n"
        << "root_rollout = " << (profile.search.rootRollout ? "true" : "false") << "\n"
        << "root_rollout_depth = " << profile.search.rootRolloutDepth << "\n"
        << "root_rollout_weight = " << profile.search.rootRolloutWeight << "\n\n"
